    ...
    public void activate(Player player, Dimoon dimoon) {
        ...
        new BukkitRunnable((Entity)entity, velocity, player, world){
            ...
            public void run() {
                ...
                this.val$world.spigot().playEffect(this.particleLocation, Effect.LARGE_SMOKE, 0, 1, 1.0f, 1.0f, 1.0f, 0.0f, 0, 64);
                this.val$world.spigot().playEffect(this.particleLocation, Effect.LARGE_SMOKE, 0, 1, 1.0f, 1.0f, 1.0f, 0.0f, 0, 64);
                this.val$world.spigot().playEffect(this.particleLocation, Effect.WITCH_MAGIC, 0, 1, 1.0f, 1.0f, 1.0f, 0.0f, 0, 64);
                this.val$world.spigot().playEffect(this.particleLocation, Effect.WITCH_MAGIC, 0, 1, 1.0f, 1.0f, 1.0f, 0.0f, 0, 64);
                ...
                if (this.particleLocation.distance(this.val$player.getEyeLocation()) < 1.5 || this.particleLocation.distance(this.val$player.getLocation()) < 1.5) {
                    ...
                    User.getUser(this.val$player.getUniqueId()).send("&7Wither's Bullet have hit you for &c" + SUtil.commaify(this.val$player.getMaxHealth() * 2.0 / 100.0) + " &7true damage.");
                    User.getUser(this.val$player.getUniqueId()).damage(this.val$player.getMaxHealth() * 2.0 / 100.0, EntityDamageEvent.DamageCause.ENTITY_ATTACK, (Entity)SkySimEngine.getPlugin().dimoon.getEntity());
                    this.cancel();
                }
            }
